...
image_addr *_correspond_image_addr(t_display *display, t_map *map, t_pos pos, t_pos player_pos)
{
    if (map->map[pos.y][pos.x] == 'E' && (pos.y != player_pos.y || pos.x != player_pos.x))
        return (display->exit.image_addr);
    else if (map->map[pos.y][pos.x] == 'E' && (pos.y == player_pos.y && pos.x == player_pos.x))
        return (display->player_on_exit.image_addr);
    else if (map->map[pos.y][pos.x] == 'P' || (pos.y == player_pos.y && pos.x == player_pos.x))
...
void    set_screen_from_map(t_all *all)
{
    ...
    while (pos.y < all->map.height)
    {
        ...
        {
            image_addr = _correspond_image_addr(&all->display, &all->map, pos, all->me.current_pos);
            mlx_put_image_to_window(all->display.mlx, all->display.mlx_win, image_addr, pos.x * BLOCK_SIZE, pos.y * BLOCK_SIZE);
            pos.x++;
        }
        pos.y++;
    }
